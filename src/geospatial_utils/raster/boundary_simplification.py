...
def extract_raster_boundary(input_path: Path, output_path: Path, output_epsg_code: int = 3857) -> Path:
    """Create a simplified raster footprint and write it to GeoJSON.

    The function runs the existing boundary pipeline and can reproject the
    generated footprint to a requested CRS.
    """
    output_path = Path(output_path)

    with tempfile.TemporaryDirectory() as tmpdir:
        temporary_dir = Path(tmpdir)
        binary_path = temporary_dir / "binary.tif"
        polygonised_path = temporary_dir / "polygonised.shp"
        simplified_path = temporary_dir / "boundary.shp"

        mask_array(Path(input_path), binary_path)
        ...
        gdal.VectorTranslate(
            ...
            str(simplified_path),
            format="GeoJSON",
            dstSRS=f"EPSG:{output_epsg_code}",
        )
...
def raster_to_footprint(raster_path: Path, output_dir: Path) -> None:
    ...
    # Create a GeoJSON footprint in the requested output directory. Keep the
    # default EPSG 3857 output to preserve current behaviour of this script.
    output_geojson = output_dir.joinpath("footprint.geojson")
    extract_raster_boundary(raster_path, output_geojson, output_epsg_code=3857)
    print("finished")
